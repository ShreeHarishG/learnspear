...
const odooAPI = {
    ...
    try {
      ...
      if (data && !data.error && data.status === 'success') {
        const userData = {
            uid: data.uid,
            name: data.name,
            partner_id: data.partner_id || 1, // Fallback to Demo ID
            session_id: data.session_id
        };
        // DEMO HACK: Force partner_id 1
        userData.partner_id = 1;
  ...
  getUser: () => {
    ...
    // DEMO HACK: Force partner_id to 1 (Administrator/Demo) if missing or for all users
    // This allows any logged-in user to see the demo data
    if (!parsedUser.partner_id) {
        parsedUser.partner_id = 1;
    }
    // Optional: Uncomment next line to force EVERYONE to be partner 1 for demo
    parsedUser.partner_id = 1; 
  ...
  deleteProduct: async (id) => {
    const response = await apiClient.delete(`/api/products?id=${id}`);
    return response.data;
  },
  ...
  deleteCustomer: async (id) => {
    const response = await apiClient.delete(`/api/customers?id=${id}`);
    return response.data;
  },
