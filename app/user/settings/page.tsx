export default function UserSettingsPage() {
    return (
        <div className="p-8">
            ...
            <p className="mt-2 text-text-muted">Manage your profile and preferences.</p>

            <div className="mt-8 max-w-2xl bg-white p-8 rounded-2xl border border-border-color shadow-sm space-y-6">
                <div>
                    <h2 className="text-lg font-semibold text-text-heading mb-4">Profile Information</h2>
                    <div className="space-y-4">
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="block text-sm font-medium text-text-heading mb-1">First Name</label>
                                <input type="text" className="w-full p-2 border border-border-color rounded-lg" defaultValue="Vishwa" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-text-heading mb-1">Last Name</label>
                                <input type="text" className="w-full p-2 border border-border-color rounded-lg" defaultValue="User" />
                            </div>
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-text-heading mb-1">Email Address</label>
                            <input type="email" className="w-full p-2 border border-border-color rounded-lg" defaultValue="vishwa@example.com" />
                        </div>
                    </div>
                </div>
                ...
                <div className="border-t border-border-color pt-6">
                    ...
                    <div className="space-y-3">
                        <label className="flex items-center gap-3">
                            ...
                            <span className="text-text-heading">Receive marketing emails</span>
                        </label>
                        <label className="flex items-center gap-3">
                            <input type="checkbox" defaultChecked className="w-4 h-4 text-primary rounded" />
                            <span className="text-text-heading">Enable two-factor authentication</span>
                        </label>
                    </div>
                </div>
